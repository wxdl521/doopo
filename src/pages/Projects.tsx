...
export default function Projects() {
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="btn-ghost">
            <Filter size={14} /> {t.projects_filter}
          </button>
          {remote.length > 0 && (
            <button
              type="button"
          ...
          <ImportProjectButton
            onImported={(p) => setImported((prev) => [p, ...prev.filter((x) => x.id !== p.id)])}
          />
        </div>
      </div>
      ...
      {error && <div className="mb-4 panel p-3 text-sm text-rose-400">加载项目失败:{error}</div>}
      ...
      {loading && remote.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          ...
          {list.length === 0 && !loading ? (
            ...
            list.map((p) => <ProjectCard key={p.id} project={p} onMenuAction={handleMenu} />)
          )}
        </div>
      )}
      ...
      <Dialog open={!!renaming} onOpenChange={(o) => !o && setRenaming(null)}>
        <DialogContent className="max-w-md">
          ...
          />
          <DialogFooter>
            <button className="btn-ghost" onClick={() => setRenaming(null)} disabled={busy}>
              {t.common_cancel}
            </button>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="text-text-muted">
                已入库到 Supabase Storage 的视频 / 故事板图文件保留,但不再关联到任何项目。
              </span>
              <br />
