        ...
        {!done ? (
          <>
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)}
                   className="input w-full mb-3" maxLength={200} />
              ...
              {VIS.map((v) => {
                ...
                return (
                  <button key={v.value} onClick={() => setVisibility(v.value)}
                    ...
                    <Icon size={16} className={active ? 'text-accent mt-0.5' : 'text-text-muted mt-0.5'} />
                    <div>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={p.onClose} className="btn-ghost text-sm">取消</button>
          ...
          <>
            ...
            <p className="text-sm text-text-secondary mb-4">
              ...
              {done.visibility === 'private' && '作品已保存为私有，可稍后在「我的发布」中改为公开。'}
            </p>
            <div className="flex items-center gap-2 panel p-2 mb-4">
              <input readOnly value={link} className="input flex-1 !bg-transparent !border-0 text-xs" />
            ...
            <div className="flex justify-end gap-2">
              <a href={link} target="_blank" rel="noreferrer" className="btn-ghost text-sm">打开作品</a>
              <button onClick={p.onClose} className="btn-primary text-sm">完成</button>
            </div>
          </>
        )}
      </div>
