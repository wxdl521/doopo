...
function buildPitchDeckPrompt(opts: {
  ...
  return [
    ...
    hasChars
      ? `- Legend (3 lines): dashed arrow + 镜头N = 镜头运动路线 (camera path), ▲ + 镜头N = 固定机位 (fixed camera position, 尖端朝向拍摄方向), solid arrow = 人物动线 (character path).`
      : `- Legend (2 lines): dashed arrow + 镜头N = 镜头运动路线 (camera path), ▲ + 镜头N = 固定机位 (fixed camera position, 尖端朝向拍摄方向).`,
...
function buildScenePrompts(
  ...
  if (data.mode === "multi-view") {
    const positive = [
      ...
      buildStyleLock(styleSpec, "scene"),
      ``,
      `[任务] 基于图1生成同一个场景的横向六宫格摄影机视图，不是六个新场景。图1是唯一的视觉与空间真值，参考图的内容优先级高于本提示词、场景名称和任何文字资料。`,
      `[地点] ${data.sceneSlug}`,
      data.sceneLocation ? `[具体地点] ${data.sceneLocation}` : "",
      data.sceneTimeOfDay ? `[时段] ${data.sceneTimeOfDay}` : "",
      data.sceneAction
        ? `[环境语义] 以下资料仅用于理解与图1相符的无生命场景信息，不能据此补充图1未出现的元素；任何角色、动物、生物或其行为均不得绘制：${data.sceneAction}`
        : "",
      EMPTY_SCENE_HARD_RULE,
      ``,
      `[画布] 3072×2048 横向画布，严格使用 2 行 × 3 列共六个等大格子；从左到右、从上到下排列。不得输出竖屏、单图、额外格子或空白格。`,
      ``,
      `【先识别图1，禁止套用默认建筑】先判断图1实际是什么空间：例如飞机客舱、室内房间、走廊、街道、自然环境或建筑外观。必须按图1的真实类别生成；图1是飞机客舱就只能生成同一架飞机的客舱视图，图1是室内就绝不能改成建筑外观、庄园、道路、庭院、山体或航拍总平面。仅当图1本身是建筑外观时，才可出现对应的外立面、道路或鸟瞰。`,
      `【场景真值锁定】六格共享图1中已经可见或可由同一连续空间直接确定的结构、比例、材质、颜色、光照、时间、天气、固定物体和空间关系。保留已见的窗、门、座椅、走廊、墙面、天花、地面、设备、建筑或自然物；不得替换主体类别、不得把室内变室外、不得把交通工具变建筑、不得换成另一处地点。`,
      `【未知区域保守原则】单张图无法确定的背面、侧面、顶部、相邻区域和外部环境一律不可自由设计。优先选择图1可验证的机位变化、同一空间内的小幅位移、反向视角或已见物体细节；若某个视角需要未知结构，宁可保持在已知空间内取景，也不要补造门窗、房间、建筑、道路、景观、天空或外景。`,
      ``,
      `【画面质量】保持图1的媒介与视觉风格；统一曝光、白平衡、焦段、景深、色彩和光影。不得把图1改成建筑可视化、地产广告或其他不同题材。`,
      ``,
      `【六格镜头规则】格子1必须是图1的忠实复现，是所有格子的锚点。格子2至格子6只允许在同一连续场景内改变摄影机位置、朝向、景别或聚焦点；每一格都必须能和格子1对上同一批结构与固定物体。`,
      ``,
      `【格子1：图1原机位锚点】忠实复现图1的摄影机位置、构图、空间类别、固定物体、材质、光线和时间。这一格必须能直接与参考图逐项对应。`,
      `【格子2：同空间左侧取景】在图1可验证的同一连续空间内轻微左移，或聚焦图1左侧已经可见的结构。不可露出未知的外部或相邻空间。`,
      `【格子3：同空间右侧取景】在图1可验证的同一连续空间内轻微右移，或聚焦图1右侧已经可见的结构。不可露出未知的外部或相邻空间。`,
      `【格子4：已知纵深或反向视角】仅沿图1已见的通道、纵深或固定结构取反向/后向视角；若背面无法确定，改用同一已知区域的另一机位，禁止补造未知结构。`,
      `【格子5：已见关键细节】拍摄图1中明确可见的关键结构、材质或固定物体的近景，例如窗、门、座椅、控制台、墙面、地面、灯具或其他已见元素；不得选择图1外的新物体。`,
      `【格子6：已知空间的宽近景】在图1已经证实的空间范围内生成更宽或更近的构图，用来补足空间关系；不得航拍、总平面或展示未经证实的顶部、外观和环境。`,
      ``,
      `[提交前检查] 严格输出横向 2×3 六宫格；格子1与图1同一场景、同一时刻、同一类别；其余五格均能与格子1验证为同一连续空间。若图1是室内或交通工具内部，六格均不得出现未经图1证实的建筑外观、庭院、道路、山体、天空、航拍或总平面。不得重新设计、增减结构或改变环境布局；无人物、动物、手、水印、logo或无关文字。`,
    ]
      .filter(Boolean)
      .join("\n");
    const negative = [
      withEmptySceneNegative(),
      "six unrelated scenes, different location, different scene category, indoor changed to outdoor, outdoor changed to indoor, vehicle interior changed to building exterior, aircraft cabin changed to mansion, palace, villa, estate, road, courtyard, aerial site plan",
      "mirrored front, flipped image, duplicated panel, same angle in all panels, flat 2D collage",
      "impossible perspective, fisheye distortion, unsupported aerial view",
      "invented room, invented building, invented door, invented window, invented road, invented landscape, invented roof, invented decoration, extra objects, removed objects, cutaway view, exploded view",
