...
export const generateImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (
      ...
      requested.toLowerCase().startsWith("azure-image2/")
    ) {
...
function buildScenePrompts(
  ...
  if (data.mode === "multi-view") {
    const positive = [
      ...
      buildStyleLock(styleSpec, "scene"),
      ``,
      `[任务] 请作为专业环境重建艺术家处理图1。图1是当前场景中选中的唯一参考图；它是空间、布局、物体和视觉风格的唯一真值。`,
      `[地点] ${data.sceneSlug}`,
      data.sceneLocation ? `[具体地点] ${data.sceneLocation}` : "",
      data.sceneTimeOfDay ? `[时段] ${data.sceneTimeOfDay}` : "",
      ``,
      `在生成之前，先对参考图进行完整的三维空间分析，并在内部建立一个一致的虚拟3D场景模型。分析并重建：摄像机位置与镜头焦距；空间长宽高比例；墙体、地面、天花板和建筑结构关系；所有主要物体的位置、尺寸、朝向；隐藏区域的合理空间延伸；材质、颜色、纹理和光照关系。`,
      ``,
      `不要重新设计场景。不要增加新的装饰。不要改变原始布局。不要创造不同风格。所有图片必须来自同一个真实存在的虚拟3D空间，只是不同摄影机角度拍摄。`,
      ``,
      `保持相同空间结构、相同物体数量、相同家具和建筑元素、相同材质、相同色彩、相同时间和光照、相同视觉风格。优先保证：空间一致性 > 物体一致性 > 视觉效果。`,
      ``,
      `[画布] 3072×2048 横向画布，严格使用 2 行 × 3 列共六个等大格子；从左到右、从上到下排列。不得输出竖屏、单图、额外格子或空白格。`,
      ``,
      `【格子1：正面平视全景】展示整体空间关系。`,
      `【格子2：45度斜侧中景】展示空间纵深、物体层次和布局关系。`,
      `【格子3：低机位仰拍】从较低高度拍摄，展示空间高度和结构比例。`,
      `【格子4：斜向高位俯拍】从较高角度观察整体布局和动线。`,
      `【格子5：垂直正俯视角】类似建筑平面观察，展示空间比例和物体位置关系。`,
      `【格子6：反方向平视视角】从相反方向观察同一个空间，展示背面结构。`,
      ``,
      `避免改变家具位置、添加不存在物体、删除原有物体、改变建筑结构、不同视角出现不同设计、透视错误、物体穿插和几何变形。`,
      ``,
      `[输出风格] 专业建筑可视化环境设计稿，真实摄影机视角，高一致性多视图展示，类似游戏环境设计验证图，干净构图，无文字说明、无标签、无UI元素。`,
    ...
    const negative = [
      ...
      "six unrelated scenes, different location, different scene category, indoor changed to outdoor, outdoor changed to indoor, vehicle interior changed to building exterior",
      "mirrored front, flipped image, duplicated panel, same angle in all panels, flat 2D collage",
      "impossible perspective, fisheye distortion, incorrect camera height, object intersection, geometry deformation",
      "invented room, invented building, invented door, invented window, invented road, invented landscape, invented roof, invented decoration, extra objects, removed objects, changed furniture position, changed architecture, changed scene layout, cutaway view, exploded view",
