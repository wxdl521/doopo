...
export function computeQuality(scenes: PipelineScene[] | undefined): SavedScript["quality"] {
  ...
  const conflictHits = allBeats.filter((b) =>
    /冲突|反转|对峙|对抗|揭穿|崩溃|对立|逼问|争吵|conflict|fight|reveal|clash|twist|confront/i.test(
      b,
    ),
  ).length;
...
export type UploadCoverFn = (input: {
  data: { scriptId: string; prompt: string };
...
export function buildCoverPrompt(
  s: Pick<
    SavedScript,
    "title" | "logline" | "premise" | "plot" | "synopsisText" | "genre" | "tone" | "type"
  >,
): string {
