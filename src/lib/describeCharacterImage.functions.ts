...
输出格式:
{
  ...
  "clothingDescription": "..."
    ...
    const modelAttempts = [data.model || 'qwen-vl-plus', 'qwen-vl-plus', 'qwen-vl-max'].filter(Boolean)
    ...
    for (const model of modelAttempts) {
      ...
      try {
        ...
        try {
          ...
          const face = (typeof parsed.faceDescription === 'string' ? parsed.faceDescription : '').trim().slice(0, 1500)
          const body = (typeof parsed.bodyDescription === 'string' ? parsed.bodyDescription : '').trim().slice(0, 1000)
          const cloth = (typeof parsed.clothingDescription === 'string' ? parsed.clothingDescription : '').trim().slice(0, 2000)
          if (!face && !body && !cloth) {
