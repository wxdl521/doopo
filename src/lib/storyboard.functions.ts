...
═══════════════════════════════════════════════════════════
你必须保证:把所有 group 的 plotText 按顺序拼接起来,**完整覆盖**原剧本的
全部内容,从第一个字到最后一个字,不跳过任何一段。

在输出 JSON 之前,强制执行以下自检:
1. 把原剧本按自然段落/场景切换/对话轮次拆成 N 个"剧情单元"。
2. 为每个剧情单元分配一个 group,确保 group 数 ≥ 剧情单元数。
3. 逐个检查:单元 1 → group[0] 的 plotText 覆盖了吗? 单元 2 → group[1]?
   ... 单元 N → group[N-1]?
4. 如果任何一个剧情单元没有被 group 覆盖 → **必须补 group**,不能合并到相邻组。
5. 特别检查原剧本的**最后一段**(结尾),必须有一个 group 专门覆盖它,
   不能因为 token 不够就省略结尾。

【分组数量硬性要求】
- 组数 = 原剧本的剧情单元数。不要为了省 token 而合并。
- 正常剧本(5 句以上对话或 2 个以上场景切换)必须 **≥ 3 组**。
- 极短剧本(仅一句话)也必须 ≥ 2 组(开头 + 结尾)。
- **禁止整集只输出 1 组**。禁止把多段剧情合并成 1 组。

每组分镜对应剧本中一段连续剧情,具体 shot 数量由你按剧情自行判断 —— 不要套固定数量,跟着内容走。
...
1. **剧情覆盖完整性(最重要)**：严格按剧本顺序切分,**必须覆盖整集全部剧情**,不得遗漏任何段落。
   开头 / 发展 / 高潮 / 结尾都要有对应的 group。输出前请逐段对照原剧本自查。
2. **最少分组数**：正常剧本必须 ≥ 3 组；极短剧本(仅一句话)也必须 ≥ 2 组；禁止整集只输出 1 组。
...
【分组数量要求 —— 2026/07 修复】
- **正常剧本必须 ≥ 3 组**。只有剧本极短(仅一句话/一个动作)才允许 2 组。
- **禁止整集只输出 1 组**。如果剧本有多个场景切换或多句对话,绝对不能合并成 1 组。
- 组数应根据剧本的实际内容量来判断:场景切换次数越多、对话轮次越多,组数应相应增加。
- 输出 JSON 前请确认:每一段剧情都被分配到了某个 group 的 plotText 中,没有遗漏。
...
===== 输出前强制自检(必须执行,否则输出无效) =====
在脑中逐段过一遍原剧本,把每个自然段落/场景切换点标出来:
  剧本开头(第 1 段): _____ → group[0] 覆盖了吗? ✓/✗
  剧本中间(第 2 段): _____ → group[1] 覆盖了吗? ✓/✗
  剧本中间(第 3 段): _____ → group[2] 覆盖了吗? ✓/✗
  ...(继续,直到剧本最后一段)
  剧本结尾(最后一段): _____ → group[N-1] 覆盖了吗? ✓/✗
如果任何一行是 ✗ → 必须补一个 group,不能跳过。输出前再检查一遍。
...
注意:
- **剧情覆盖检查(最重要)**：输出 JSON 后请逐句对照原剧本自查 —— 开头、中间每一段、结尾是否都有对应的 group。**严禁遗漏任何一段剧情**。
- **最少 3 组(硬性要求)**：正常剧本必须 ≥ 3 组；极短剧本(仅一句话)也必须 ≥ 2 组；禁止整集只输出 1 组。
