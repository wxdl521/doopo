...
【分组数量硬性要求 —— 2026/07 用户反馈修复】
- **至少生成 2 组分镜**。即使剧本很短,也要尽量拆成 2 组(开头 + 结尾)。
- 正常长度的剧本(5 句以上对话或 2 个以上场景切换)必须 **≥ 3 组**。
- 只有一种例外可以只生成 1 组:剧本确实只有一句话/一个动作,无法再切分。
- 严禁为了省事把多段剧情合并成 1 组。

【剧情覆盖完整性 —— 同样重要】
- 你必须**逐句检查**原剧本,确保每一段剧情、每一次场景切换、每一个关键对话
  都被分配到了某个 group 的 plotText 中,**不能遗漏任何一段**。
- 输出 JSON 后请在脑中过一遍:原剧本开头 → 你的 group[0] 覆盖了吗?
  原剧本中间部分 → 你的 group[1..n-1] 覆盖了吗? 原剧本结尾 → 你的最后一个 group 覆盖了吗?
- 如果发现某段剧情没被覆盖,**必须补一个 group**,不要跳过。
- plotText 中必须**完整引用原剧本的全部台词**,不能只引用一部分就跳过。
