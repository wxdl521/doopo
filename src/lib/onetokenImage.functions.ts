...
const ONETOKEN_GPT_IMAGE2_SIZES = new Set(["1024x1024", "1024x1792", "1792x1024"]);
...
export async function callOnetokenImage(input: OnetokenImageInput): Promise<OnetokenImageResult> {
  ...
  try {
    ...
    if (!res || !res.ok) {
      ...
      console.warn(
        `[onetoken×] model=${model} status=${status} dur=${Date.now() - t0}ms body=${lastText.slice(0, 200)}`,
      );
      return {
        url: "",
        urls: [],
        error: `[onetoken ${model}] ${status}: ${lastText.slice(0, 300)}`,
        model,
      };
    }
    ...
    if (urls.length === 0) {
      console.warn(
        `[onetoken×] model=${model} empty-data dur=${Date.now() - t0}ms raw=${rawText.slice(0, 400)}`,
      );
      return {
        url: "",
        urls: [],
        error: `[onetoken ${model}] no image returned: ${rawText.slice(0, 200) || "empty data"}`,
        model,
      };
    }
    ...
    clearTimeout(timeout);
    console.warn(
      `[onetoken×] model=${model} network dur=${Date.now() - t0}ms err=${e instanceof Error ? e.message : "fetch failed"}`,
    );
    return {
      url: "",
      urls: [],
      error: `[onetoken ${model}] network: ${e instanceof Error ? e.message : "fetch failed"}`,
      model,
    };
  }
}
