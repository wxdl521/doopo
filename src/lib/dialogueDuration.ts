...
//  台词时长估算(2026/07 新增)
//
//  背景:AI 切分一集剧情 -> 若干分镜组,每组 = 一段 ≤10s 的视频。视频生成时
//  会把 plotText 里的台词"说出来"(配音)。如果一组塞的台词太多,10s 说不完,
//  视频模型就会漏台词 / 语序发音乱。这里提供按"字数 × 语速"估算台词说完
//  所需秒数的工具,给两处用:
//    1) storyboard.functions.ts 切分时:给 prompt 里的"台词可说完性"预算提供
//       口径,并在 normalizeGroup 里给超 10s 的组打 dialogueOverloadSec 标记。
