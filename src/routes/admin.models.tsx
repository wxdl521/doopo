...
function AdminModels() {
  ...
  const modalityLabel: Record<AdminModel["modality"], string> = {
    text: t.admin_modality_text,
    image: t.admin_modality_image,
    video: t.admin_modality_video,
    audio: t.admin_modality_audio,
  };
  const statusLabel: Record<AdminModel["status"], string> = {
    online: t.common_online,
    degraded: t.common_degraded,
    offline: t.common_offline,
  };
  return (
    <>
      <PageHeader
        ...
        actions={
          <button className="btn-primary">
            <Plus size={14} /> {t.admin_add_provider}
          </button>
        }
      />
            ...
            {mockAdminModels.map((m) => (
              <tr key={m.id} className="border-t border-border">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs border ${modalityTone[m.modality]}`}
                  >
                ...
                <td className="px-4 py-3 text-right font-mono">
                  {m.status === "offline" ? "—" : `${m.latencyMs} ms`}
                </td>
                ...
                <td className="px-4 py-3 font-mono text-xs flex items-center gap-1">
                  <KeyRound size={12} className="text-text-muted" /> {m.apiKeyMasked}
                </td>
                <td className="px-4 py-3 text-right">
                  <button className="btn-ghost !py-1 !px-3 text-xs">{t.common_edit}</button>
                </td>
              </tr>
            ))}
          </tbody>
