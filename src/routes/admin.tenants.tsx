...
function AdminTenants() {
  ...
  const statusLabel: Record<Tenant['status'], string> = {
    active: t.common_active, pending: t.common_pending, suspended: t.common_suspended,
  }
  return (
    <>
          ...
          <tbody>
            {items.map((tn) => (
              <tr key={tn.id} className="border-t border-border">
                ...
                <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-full text-xs border ${statusTone[tn.status]}`}>{statusLabel[tn.status]}</span></td>
                <td className="px-4 py-3 text-text-muted">{tn.created}</td>
                <td className="px-4 py-3 font-mono text-xs">{tn.contact}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex gap-2 justify-end">
                    {tn.status !== 'active' && <button onClick={() => setStatus(tn.id, 'active')} className="btn-primary !py-1 !px-3 text-xs">{t.admin_activate}</button>}
                    {tn.status !== 'suspended' && <button onClick={() => setStatus(tn.id, 'suspended')} className="btn-ghost !py-1 !px-3 text-xs">{t.admin_suspend}</button>}
                  </div>
                </td>
              </tr>
            ))}
