...
function ShowcaseDetailPage() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      >
        <ArrowLeft size={14} /> {t.shd_back}
      </Link>
      <div
        className={`rounded-2xl overflow-hidden bg-gradient-to-br ${meta.gradient} aspect-video mb-6`}
      />
      <div className="flex items-start justify-between gap-4 mb-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`btn-ghost ${liked ? "!text-rose-500" : ""}`}
          >
            <Heart size={14} fill={liked ? "currentColor" : "none"} /> {likes}
          </button>
          <button className="btn-ghost">
            <Share2 size={14} /> {t.shd_share}
          </button>
        </div>
      </div>
      ...
      <ul className="space-y-3 max-w-3xl">
        {detail.comments.length === 0 && (
          <li className="text-sm text-text-muted">{t.shd_no_comments}</li>
        )}
        {detail.comments.map((c) => (
          <li key={c.id} className="panel p-4">
            <div className="flex items-center justify-between text-sm">
              <span className="font-semibold">{c.author}</span>
              <span className="text-xs text-text-muted">{c.ts}</span>
            </div>
