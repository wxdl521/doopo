    ...
    <>
      ...
      {projects.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {projects.map((p) => (
            <Link key={p.id} to="/workspace/$workspaceId" params={{ workspaceId: p.id }} className="card group">
              <div className="aspect-[16/10] bg-gradient-to-br from-indigo-700 via-violet-800 to-slate-950 relative flex items-center justify-center overflow-hidden">
                {p.customCover ? (
                  <img src={p.customCover} alt={p.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
      ...
      {characters.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {characters.map((c) => (
            <Link key={c.id} to="/assets/$tab/$id" params={{ tab: 'character', id: c.id }} className="card overflow-hidden">
              ...
              <div className="p-3">
                ...
                <div className="text-xs text-text-muted truncate">{c.role_label || c.role || ''}</div>
              </div>
            </Link>
          ))}
        </div>
