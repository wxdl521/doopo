...
import { getPost, recordView, toggleLike, isLiked, type CommunityPost } from '@/lib/community.functions'
...
function PostPage() {
  ...
  useEffect(() => {
    ...
    checkLiked({ data: { postId } }).then((r) => setLiked(r.liked)).catch(() => {})
  ...
  if (!post) {
    return (
      <div className="p-10 text-center text-text-muted">
        作品不存在或未公开。
        <Link to="/community" className="ml-2 text-accent">返回社区</Link>
      </div>
    )
  }
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto">
      <Link to="/community" className="inline-flex items-center gap-1 text-sm text-text-muted hover:text-accent mb-4">
        <ArrowLeft size={14} /> 返回社区
      </Link>
      <div className={`rounded-2xl overflow-hidden aspect-video mb-6 ${cover?.startsWith('bg-') ? cover : 'bg-gradient-to-br from-indigo-700 via-violet-800 to-slate-950'}`}
...
function ScriptBody({ payload }: { payload: unknown }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {s.synopsisText && (
        <div className="panel p-4 whitespace-pre-wrap text-sm leading-relaxed">{s.synopsisText}</div>
      )}
        ...
        <div className="space-y-3">
          {s.scenes.map((sc, i) => (
            <div key={i} className="panel p-4">
              ...
              {sc.dialogue?.map((d, j) => (
                <div key={j} className="text-sm"><span className="font-semibold">{d.role}：</span>{d.line}</div>
              ))}
            </div>
          ))}
        </div>
...
function GenericBody({ payload }: { payload: unknown }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <pre className="panel p-4 text-xs font-mono overflow-x-auto">{JSON.stringify(obj, null, 2)}</pre>
    </div>
  )
}