...
import { listMyPosts, updatePostVisibility, deletePost, type PostVisibility } from '@/lib/community.functions'
...
function MyPostsPage() {
  ...
  useEffect(() => {
    list().then((r) => setRows(r)).finally(() => setLoading(false))
  }, [list])
          ...
          {rows.map((r) => (
            <div key={r.id} className="panel p-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <Link to="/community/$postId" params={{ postId: r.id }}
                      className="font-semibold truncate hover:text-accent">
                  {r.title}
                </Link>
                <div className="text-xs text-text-muted mt-0.5">
                  {r.kind} · ♥ {r.likes_count} · 👁 {r.views_count} · {new Date(r.created_at).toLocaleDateString()}
                </div>
              </div>
              <div className="flex items-center gap-1 text-xs">
                <VisButton active={r.visibility === 'public'} icon={Globe} label="公开"
                  onClick={() => changeVis(r.id, 'public')} />
                <VisButton active={r.visibility === 'unlisted'} icon={Link2} label="仅链接"
                  onClick={() => changeVis(r.id, 'unlisted')} />
                <VisButton active={r.visibility === 'private'} icon={Lock} label="私有"
...
function VisButton({ active, icon: Icon, label, onClick }: {
  active: boolean; icon: typeof Globe; label: string; onClick: () => void
}) {
  return (
