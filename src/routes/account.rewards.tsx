...
function Rewards() {
  ...
  const typeLabel: Record<RewardEntry['type'], string> = {
    earn: t.account_reward_earn, spend: t.account_reward_spend, cashout: t.account_reward_cashout,
  }
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <StatCard icon={TrendingUp} label={t.account_lifetime_earned} value={earned} tone="success" />
        <StatCard icon={Award} label={t.account_tier} value={t.account_tier_value} hint={t.account_tier_top} />
      </div>

      <section className="panel p-6 mb-6">
        <h3 className="font-display font-bold mb-3">{t.account_tier_progress}</h3>
        <div className="text-xs text-text-muted mb-2 flex justify-between"><span>{t.account_tier_l4}</span><span>{t.account_tier_l5_to_go}</span></div>
        <div className="h-3 bg-bg-elevated rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-accent to-accent-soft" style={{ width: '64%' }} />
        </div>
      </section>
          ...
          <tbody>
            {mockRewards.map((r) => (
              <tr key={r.id} className="border-t border-border">
                ...
                <td className={`px-4 py-3 text-right font-mono ${r.points >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{r.points >= 0 ? `+${r.points}` : r.points}</td>
              </tr>
            ))}
          </tbody>
        </table>
