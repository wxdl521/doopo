  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const prompt = url.searchParams.get('prompt') || 'A cute orange cat sitting on a wooden table';

        const results: Record<string, unknown> = {};

        // 1. Standard OpenAI images/generations
        results['images/generations'] = await tryCall('/v1/images/generations', {
          model, prompt, n: 1, size: '1024x1024',
        }, key);

        // 2. Chat completions (vision-style) fallback
        results['chat/completions'] = await tryCall('/v1/chat/completions', {
        ...
        try { mjson = JSON.parse(mtext); } catch {}
